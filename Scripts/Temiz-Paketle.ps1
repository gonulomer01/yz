...
$HaricTutulanlar = @("bin", "obj", ".vs", ".git", "GeminiChromeProfile_*", "ChatGptChromeProfile_*", "CopilotChromeProfile_*", "*.db", "*.db-shm", "*.db-wal", "*.mdf", "*.ldf", "ai_automation_project_*", "*.zip", "github_push.ps1", "sync_repos.ps1")
...
# Dahili betikleri temiz kopyadan çıkarıyoruz
if (Test-Path "$HedefKlasor\Scripts\github_push.ps1") { Remove-Item "$HedefKlasor\Scripts\github_push.ps1" -Force }
if (Test-Path "$HedefKlasor\Scripts\sync_repos.ps1") { Remove-Item "$HedefKlasor\Scripts\sync_repos.ps1" -Force }
...
$TemizCreds = [ordered]@{
    ...
    ChatGptAccounts = @(
        [ordered]@{ Id = 1; ProfileName = "ChatGptChromeProfile_1"; AccountLabel = "ChatGPT Hesap #1 (Ana Profil)"; Status = "Active"; LastUsed = "" }
    )
    CurrentChatGptProfileIndex = 0
    CopilotAccounts = @(
        [ordered]@{ Id = 1; ProfileName = "CopilotChromeProfile_1"; AccountLabel = "Copilot Hesap #1 (Ana Profil)"; Status = "Active"; LastUsed = "" }
    )
    CurrentCopilotProfileIndex = 0
}
